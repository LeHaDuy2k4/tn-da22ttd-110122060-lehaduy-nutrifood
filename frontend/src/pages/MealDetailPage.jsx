...
import { useAuthStore } from "@/stores/useAuthStores"; // Đảm bảo đường dẫn này đúng
...
const MealDetailPage = () => {
  ...
  useEffect(() => {
    const fetchMealDetails = async () => {
      try {
        setIsLoading(true);
        // Gọi API chi tiết món ăn (Giả định bạn có endpoint này ở backend)
        const mealRes = await axios.get(`http://localhost:5001/api/meals/${id}`);
        setMeal(mealRes.data);

        // Nếu user đã đăng nhập, kiểm tra xem món này đã thả tim chưa
        if (user) {
        ...
        navigate('/menu'); // Quay về menu nếu lỗi
      } finally {
        setIsLoading(false);
      }
    };
            ...
            <form onSubmit={handleLogMeal} className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-xs text-slate-400 font-medium">x {meal.totalNutrition?.calories} kcal</span>
                </div>
              </div>
