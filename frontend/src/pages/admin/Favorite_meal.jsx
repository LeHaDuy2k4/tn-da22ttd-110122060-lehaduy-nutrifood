...
const Favorite_meal = () => {
  ...
  const fetchAllFavorites = async () => {
    try {
      ...
      // Gọi API lấy toàn bộ dữ liệu thả tim của hệ thống
  ...
  // 3. XỬ LÝ AN TOÀN HIỂN THỊ DỮ LIỆU CÓ POPULATE
  ...
  const filteredFavorites = favorites.filter(fav => {
    ...
    const matchUser = fav.userId?.name?.toLowerCase().includes(searchLower) || false;
    const matchMeal = fav.mealId?.name?.toLowerCase().includes(searchLower) || false;
    
    return matchUser || matchMeal;
  });
                  ...
                  currentItems.map((fav, index) => (
                    <tr key={fav._id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold shadow-sm border border-slate-200 shrink-0">
                            {renderUserInfo(fav.userId).charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-slate-900">{renderUserInfo(fav.userId)}</p>
                            <span className="text-[11px] font-medium text-slate-500">ID: {fav.userId?._id?.slice(-5) || 'N/A'}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            onClick={() => handleDelete(fav._id, renderUserInfo(fav.userId), renderMealInfo(fav.mealId))}
