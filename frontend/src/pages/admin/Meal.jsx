...
const Meals = () => {
  ...
  const initialFormState = { 
    ...
    imageUrl: '',
    categoryIds: [],
    ingredients: [], // Dạng: [{ ingredientId: '', quantity: 1, unit: 'gam' }]
    instructions: [''], // Mảng string các bước
    prepTime: 0,
    cookTime: 0,
    servings: 1,
    isActive: true 
  ...
  const handleSubmit = async (e) => {
    ...
    const cleanedData = { ...formData, ingredients: validIngredients, instructions: validInstructions };
    ...
    try {
      ...
      if (editId) {
        await axios.put(`http://localhost:5001/api/meals/${editId}`, cleanedData, config);
        toast.success("Cập nhật món ăn thành công!");
      } else {
        await axios.post('http://localhost:5001/api/meals', cleanedData, config);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fadeIn overflow-y-auto">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl my-10 animate-slideUp">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[70vh] space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">URL Hình ảnh</label>
                    <input type="text" value={formData.imageUrl} onChange={e => setFormData({...formData, imageUrl: e.target.value})} placeholder="https://..." className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none" />
                  </div>
                  <div>
            ...
            <div className="sticky bottom-0 p-6 border-t border-slate-100 bg-white rounded-b-[2rem] flex gap-3">
              <button type="button" onClick={closeModal} className="flex-1 py-3 bg-slate-100 text-slate-700 rounded-xl font-bold hover:bg-slate-200">Hủy</button>
              <button onClick={handleSubmit} disabled={isSubmitting} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 flex items-center justify-center gap-2">
